...
public abstract class CollectionsKt__CollectionsKt extends AbstractC1167eq {
    /* renamed from: a */
    public static final void m5581a(int i, int i2, int i3) {
        if (i2 <= i3) {
            if (i2 >= 0) {
                if (i3 <= i) {
                    return;
            ...
            throw new IndexOutOfBoundsException(ye0.m8292l(i2, "fromIndex (", ") is less than zero."));
        }
    ...
    public static final <T> ArrayList<T> arrayListOf(@NotNull T... elements) {
        ...
        return new ArrayList<>(new C1976xa(elements, true));
    }

    @NotNull
    public static final <T> Collection<T> asCollection(@NotNull T[] tArr) {
        Intrinsics.checkNotNullParameter(tArr, "<this>");
        return new C1976xa(tArr, false);
    }
    ...
    public static final <T extends Comparable<? super T>> int binarySearch(@NotNull List<? extends T> list, @Nullable T t, int i, int i2) {
        ...
        m5581a(list.size(), i, i2);
        int i3 = i2 - 1;
        while (i <= i3) {
            int i4 = (i + i3) >>> 1;
            int compareValues = AbstractC1919vr.compareValues(list.get(i4), t);
            if (compareValues < 0) {
                i = i4 + 1;
            } else {
                if (compareValues <= 0) {
    ...
    public static final <T> List<T> listOfNotNull(@Nullable T t) {
        return t != null ? AbstractC1167eq.listOf(t) : emptyList();
    }
    ...
    public static <T> List<T> optimizeReadOnlyList(@NotNull List<? extends T> list) {
        ...
        if (size != 0) {
            if (size == 1) {
                return AbstractC1167eq.listOf(list.get(0));
            }
            return list;
        }
        return emptyList();
