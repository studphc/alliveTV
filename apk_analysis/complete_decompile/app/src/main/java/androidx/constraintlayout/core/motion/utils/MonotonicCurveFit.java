...
public class MonotonicCurveFit extends CurveFit {
    ...
    /* renamed from: d */
    public final boolean f2200d = true;
    ...
    public MonotonicCurveFit(double[] dArr, double[][] dArr2) {
        ...
        this.f2201e = new double[length2];
        int i = length - 1;
    ...
    /* renamed from: a */
    public static double m443a(double d, double d2, double d3, double d4, double d5, double d6) {
        double d7 = d2 * d2;
        double d8 = d2 * 6.0d;
    ...
    /* renamed from: b */
    public static double m444b(double d, double d2, double d3, double d4, double d5, double d6) {
        double d7 = d2 * d2;
        double d8 = d7 * d2;
        double d9 = 3.0d * d7;
    ...
    public void getPos(double d, double[] dArr) {
        double[] dArr2 = this.f2197a;
        int length = dArr2.length;
        double[][] dArr3 = this.f2198b;
        int i = 0;
        int length2 = dArr3[0].length;
        if (this.f2200d) {
            double d2 = dArr2[0];
            double[] dArr4 = this.f2201e;
            if (d <= d2) {
                getSlope(d2, dArr4);
                for (int i2 = 0; i2 < length2; i2++) {
        ...
        while (i6 < length - 1) {
            ...
            if (d < d4) {
                ...
                while (i < length2) {
                    ...
                    double[][] dArr5 = this.f2199c;
                    dArr[i] = m444b(d6, d7, d8, d9, dArr5[i6][i], dArr5[i8][i]);
                    i++;
                }
                return;
            }
        ...
        while (i < length - 1) {
            ...
            if (d2 <= d3) {
                ...
                for (int i3 = 0; i3 < length2; i3++) {
                    ...
                    double[][] dArr4 = this.f2199c;
                    dArr[i3] = m443a(d5, d6, d7, d8, dArr4[i][i3], dArr4[i2][i3]) / d5;
                }
                return;
            }
            i = i2;
    ...
    public double getSlope(double d, int i) {
        ...
        while (i2 < length - 1) {
            ...
            if (d2 <= d3) {
                double d4 = dArr[i2];
                double d5 = d3 - d4;
                double[][] dArr2 = this.f2198b;
                double d6 = dArr2[i2][i];
                double d7 = dArr2[i3][i];
                double[][] dArr3 = this.f2199c;
                return m443a(d5, (d2 - d4) / d5, d6, d7, dArr3[i2][i], dArr3[i3][i]) / d5;
            }
            i2 = i3;
        }
        return 0.0d;
        ...
        while (i6 < length - 1) {
            ...
            if (d < d4) {
                ...
                while (i < length2) {
                    ...
                    double[][] dArr4 = this.f2199c;
                    fArr[i] = (float) m444b(d6, d7, d8, d9, dArr4[i6][i], dArr4[i8][i]);
                    i++;
                }
                return;
            }
    ...
    public double getPos(double d, int i) {
        double[] dArr = this.f2197a;
        int length = dArr.length;
        boolean z = this.f2200d;
        double[][] dArr2 = this.f2198b;
        int i2 = 0;
        if (z) {
            double d2 = dArr[0];
            if (d <= d2) {
                return (getSlope(d2, i) * (d - d2)) + dArr2[0][i];
            }
            int i3 = length - 1;
            double d3 = dArr[i3];
            if (d >= d3) {
                return (getSlope(d3, i) * (d - d3)) + dArr2[i3][i];
            }
        } else {
            if (d <= dArr[0]) {
                return dArr2[0][i];
        ...
        while (i2 < length - 1) {
            ...
            if (d < d5) {
                ...
                double[][] dArr3 = this.f2199c;
                return m444b(d6, d7, d8, d9, dArr3[i2][i], dArr3[i5][i]);
            }
            i2 = i5;
        }
        return 0.0d;
